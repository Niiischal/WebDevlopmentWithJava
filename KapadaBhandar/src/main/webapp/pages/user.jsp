...
<body>
	...
	<sql:query var="users" dataSource="${dbconnection}">
          	SELECT userID FROM registration WHERE username= '${user}' AND password = '${encPassword}';
          </sql:query>
	...
	<div class="nav">
		...
		<div class="nav-links">
			...
			if (mySession.checkUser(user)) {
			%>
			<a href="cart.jsp">View Cart</a> <a href="">Profile</a>
			<%
			}
			%>
			<form
	...
	SELECT firstName, lastName, address, email, username, password, image FROM registration WHERE userID = '${firstRow.userID}';
	</sql:query>
		...
		<div class="userprofileForm">
			<form action="../UpdateUser" method="post"
				...
				<input class="box" placeholder="${userData.firstName}"
					name="firstName" /> <input class="box"
					placeholder="${userData.lastName}" name="lastName"> <input
					class="box" placeholder="${userData.address}" name="address">
				<input class="box" placeholder="${userData.email}" name="email">
				<input class="box" placeholder="${userData.username}"
					name="username"> <input class="box" placeholder="Password"
					name="password"> <input type="file" class="box"
					accept="image/*" name="image"> <input type="submit"
					value="Update Profile" class="btn" />
			</form>
		</div>
		<div class="image">
